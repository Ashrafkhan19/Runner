...
class ActiveRunService : Service() {
    ...
    private val baseNotification by lazy {
        NotificationCompat
            ...
            .setContentTitle(getString(com.example.run.presentation.R.string.active_run))
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> {
                ...
                start(Class.forName(activityName))
    ...
    private fun createNotificationChannel() {
        val channel = NotificationChannel(
            CHANNEL_ID, getString(com.example.run.presentation.R.string.active_run),
    ...
    companion object {
        ...
        fun createStartIntent(context: Context, activityClass: Class<*>): Intent {
            return Intent(context, ActiveRunService::class.java).apply {
                action = ACTION_START
                putExtra(EXTRA_ACTIVITY_NAME, activityClass.name)
            }
        }

        fun createSopIntent(context: Context): Intent {
            return Intent(context, ActiveRunService::class.java).apply {
                action = ACTION_STOP
            }
        }
